...
bot = telebot.TeleBot('7402185479:AAEYq2M820yISDSG5xUX6B--UyyBHskGOuc')

# Join :- https://t.me/freebgmiddos045 # Admin user IDs
admin_id = ["1692589043","5628671567"]
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        # Join :- @naamhaidev03 # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Join :- @naamhaidev03 # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 10:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @naamhaidev03 # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- https://t.me/+OaDUxEhnsqNiNTI1 # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- https://t.me/+OaDUxEhnsqNiNTI1 # Convert time to integer
            time = int(command[3])  # Join :-  https://t.me/+OaDUxEhnsqNiNTI# Convert port to integer
            if time > 500:
                response = "𝗘𝗿𝗿𝗼𝗿: 𝗧𝗶𝗺𝗲 𝗶𝗻𝘁𝗲𝗿𝘃𝗮𝗹 𝗺𝘂𝘀𝘁 𝗯𝗲 𝗹𝗲𝘀𝘀 𝘁𝗵𝗮𝗻 500."
            else:
            ...
            response = "🤦‍♂️𝙐𝙨𝙖𝙜𝙚: /𝙗𝙜𝙢𝙞 <𝙞𝙥> <𝙥𝙤𝙧𝙩> <𝙩𝙞𝙢𝙚_𝙨𝙚𝙘𝙤𝙣𝙙𝙨>\n\n🤷‍♀️𝙀𝙭𝙖𝙢𝙥𝙡𝙚  /bgmi 20.235.94.237 17870 300"  # Join :- @naamhaidev03 # Updated command syntax
    else:
        response = " 𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙\n𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩\n𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢 @naamhaidev03 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨"
...
𝟯. 𝗪𝗲 𝗗𝗮𝗶𝗹𝘆 𝗖𝗵𝗲𝗰𝗸𝘀 𝗧𝗵𝗲 𝗟𝗼𝗴𝘀 𝗦𝗼 𝗙𝗼𝗹𝗹𝗼𝘄 𝘁𝗵𝗲𝘀𝗲 𝗿𝘂𝗹𝗲𝘀 𝘁𝗼 𝗮𝘃𝗼𝗶𝗱 𝗕𝗮𝗻!!@naamhaidev03'''
    bot.reply_to(message, response)
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f''' ☑️ 𝘾𝙤𝙣𝙩𝙖𝙘𝙩 𝙁𝙤𝙧 @naamhaidev03 𝙏𝙤 𝙂𝙚𝙩 ??𝙘𝙘𝙚𝙨𝙨 🍾
'''
    bot.reply_to(message, response)
...
def broadcast_message(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝗢𝗡𝗟𝗬 𝗢𝗪𝗡𝗘𝗥 𝗖𝗔𝗡 𝗨𝗦𝗘 @naamhaidev03."
